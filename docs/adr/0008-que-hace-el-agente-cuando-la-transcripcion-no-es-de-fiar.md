...
Y hay un segundo camino, peor, porque no necesita que la sala esté en silencio:
el buffer que se transcribe lleva pegada la **cola de silencio que cerró el
turno** (300 ms, o 1200 con la ventana larga, o 3600 con las dos reanudaciones
que el sistema permite). Sobre esa cola, Whisper llegó a repetir la frase
entera detrás de sí misma: **15 palabras de más** en un turno real.
...
sintético: son tramos de medio segundo sacados de las propias grabaciones —los
que están por debajo del 10% de la energía de su fichero— cosidos en clips de
1, 2, 4 y 8 segundos.
...
| turnos que ganan palabras por la cola | 1 de 18 (+15) | 0 de 18 |
| latencia, turno de 3 s + 1,2 s, mediana n=18 | 178 ms | 202 ms (**+24 ms**) |

Desglose de los 7: ninguno a 1 s, dos a 2 s, uno a 4 s, cuatro a 8 s. **Cuanto
más silencio se le da, más se inventa.** Y ninguno de los 24 clips pasados por
la línea telefónica alucinó: la banda de 300–3400 Hz se lleva por delante la
parte del ruido que el modelo confunde con voz.
...
umbral, y ahí **2 de 12 clips alucinan a los 2 segundos**, que es una duración
de turno perfectamente normal.
...
- **`no_speech_prob` no separa.** Hay habla de verdad que llega a 0,93 y los
  inventos se quedan entre 0,50 y 0,75. La señal que parece hecha para esto es
  la que no sirve.
- **`avg_logprob` separaba… hasta que entró más material.** Cazaba 7 de 7 con
  ...
  Un guardia que se degrada al ver material nuevo no se pone en la ruta de
  nadie con n=7.
...
caza 5 de 7. Mejor que nada y peor que el filtro, con una regla más que
explicar.
...
2. **Las tres señales se resumen y se pegan al aviso del turno**
   ...
   vez de con siete casos. Se guardan los extremos y no el promedio, porque lo
...
- **Una sola sala y un solo micrófono.** 7 de 64 es una tasa de esta
